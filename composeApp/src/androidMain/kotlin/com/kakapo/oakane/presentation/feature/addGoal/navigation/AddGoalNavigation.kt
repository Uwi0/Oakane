...
fun NavGraphBuilder.addGoalScreen(
    ...
    val route = "$ADD_GOAL_ROUTE/{${NavigationArgs.GOAL_ID_ARGS}}"
    val arguments = listOf(navArgument(NavigationArgs.GOAL_ID_ARGS) { type = NavType.LongType })
    composable(route = route, arguments = arguments) { backStackEntry ->
        val backStackArguments = requireNotNull(backStackEntry.arguments)
        val goalId = backStackArguments.getLong(NavigationArgs.GOAL_ID_ARGS)
